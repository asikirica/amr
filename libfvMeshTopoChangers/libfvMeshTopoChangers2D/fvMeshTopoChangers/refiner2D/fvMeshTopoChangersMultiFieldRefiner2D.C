...
#include "fvMeshTopoChangersMultiFieldRefiner2D.H"
...
{
    defineTypeNameAndDebug(multiFieldRefiner2D, 0);
    addToRunTimeSelectionTable(fvMeshTopoChanger, multiFieldRefiner2D, fvMesh);
}
}
...
Foam::label Foam::fvMeshTopoChangers::multiFieldRefiner2D::count
(
...
void Foam::fvMeshTopoChangers::multiFieldRefiner2D::calculateProtectedCells
(
...
void Foam::fvMeshTopoChangers::multiFieldRefiner2D::readDict()
{
...
Foam::fvMeshTopoChangers::multiFieldRefiner2D::refine
(
...
Foam::autoPtr<Foam::polyTopoChangeMap>
Foam::fvMeshTopoChangers::multiFieldRefiner2D::unrefine
(
...
)
{
...

    {
        forAll(splitEdges, i)
        {
            const label edgeI = splitEdges[i];

            const edge& e = mesh().edges()[edgeI];

            forAll(e, j)
            {
                const label pointi = e[j];
...
Foam::word Foam::fvMeshTopoChangers::multiFieldRefiner2D::Uname
(
...
void Foam::fvMeshTopoChangers::multiFieldRefiner2D::refineFluxes
(
...
void Foam::fvMeshTopoChangers::multiFieldRefiner2D::refineUfs
(
    const labelHashSet& masterFaces,
    const polyTopoChangeMap& map
)
...
void Foam::fvMeshTopoChangers::multiFieldRefiner2D::unrefineFluxes
(
...
void Foam::fvMeshTopoChangers::multiFieldRefiner2D::unrefineUfs
(
    const Map<label>& faceToSplitPoint,
    const polyTopoChangeMap& map
)
...
const Foam::cellZone& Foam::fvMeshTopoChangers::multiFieldRefiner2D::findCellZone
(
...
void Foam::fvMeshTopoChangers::multiFieldRefiner2D::geometryRefineCandidates
(
    PackedBoolList& candidateCells,
    const dictionary& fieldDict
) const
{
    if (!fieldDict.found("surface"))
    {
        FatalErrorInFunction
            << "Surface file not specified." << nl
            << exit(FatalError);
    }

    const fileName surfaceName(fieldDict.lookup("surface"));

    const word mode(fieldDict.lookupOrDefault<word>("mode", "inside"));

    const word operation(fieldDict.lookupOrDefault<word>("operation", "AND")); 

    const scalar buffer(fieldDict.lookupOrDefault<scalar>("buffer", 0.0));

    if (mode != "inside" && mode != "outside")
    {
        FatalErrorInFunction
            << "Invalid geometry mode specified." << nl
            << "Valid options are \"inside\" and \"outside\"." << nl
            << exit(FatalError);
    }

    if (operation != "default" && operation != "AND" && operation != "OR" && operation != "NOT")
    {
        FatalErrorInFunction
            << "Unknown operation \"" << operation << "\"." << nl
            << "Valid operations are \"AND\", \"OR\", \"NOT\" and \"default\"." << nl
            << exit(FatalError);
    }

    // Read geometry
    autoPtr<searchableSurface> surfacePtr = searchableSurface::New
    (
        "triSurfaceMesh",
        IOobject
        (
            surfaceName,
            mesh().time().constant()/"triSurface",
            mesh(),
            IOobject::MUST_READ,
            IOobject::NO_WRITE
        ),
    ...
    if (!surfacePtr().hasVolumeType())
    {
        FatalErrorInFunction
            << "Surface " << surfaceName << " does not support inside/outside testing." << nl
            << exit(FatalError); 
    }

    // Get cell centers and geometry bounds
    const vectorField cellCenters(mesh().C());
    const boundBox& surfaceBounds = surfacePtr().bounds();
    const point surfaceCenter = surfaceBounds.midpoint();

    // Process points
    List<pointIndexHit> nearest(cellCenters.size());
    vectorField normals(cellCenters.size());

    surfacePtr().findNearest
    (
        cellCenters,
        scalarField(cellCenters.size(), GREAT),
        nearest
    );
    surfacePtr().getNormal(nearest, normals);
    
    // Process cells with computed normals
    forAll(cellCenters, celli)
    {
        if(!nearest[celli].hit())
        {
            continue;
        }

        vector& n = normals[celli];
        const point& p = cellCenters[celli];
        const point& hitPt = nearest[celli].hitPoint();

        // Compute normal direction
        if(((hitPt - surfaceCenter) & n) < 0)
        {
            n *= -1;
        }

        // Check if point is inside or outside
        bool isInside = ((p - hitPt) & n) < 0;
        scalar distToSurface = mag(p - hitPt);

        // Mark cells based on mode and buffer
        bool markCell = false;
        if(buffer > 0)
        {
            if(mode == "inside")
            {
                markCell = isInside && distToSurface >= buffer;
            }
            else
            {
                markCell = !isInside && distToSurface >= buffer;
        ...
        {
            markCell = (mode == "inside") ? isInside : !isInside;
        }

        if(markCell)
        {
            candidateCells.set(celli);
        }
    }
}


void Foam::fvMeshTopoChangers::multiFieldRefiner2D::processFieldRefinement
(
    PackedBoolList& candidateCells,
...
) const
{
    ...
    const scalar lowerRefineLevel =
        fieldDict.lookup<scalar>("lowerRefineLevel");
    const scalar upperRefineLevel =
        fieldDict.lookup<scalar>("upperRefineLevel");

    if (lowerRefineLevel >= upperRefineLevel)
    {
        FatalErrorInFunction
            << "Invalid refinement levels for field " << fieldName << "." << nl
            << "lowerRefineLevel (" << lowerRefineLevel
            << ") must be less than upperRefineLevel ("
            << upperRefineLevel << ")." << nl
            << exit(FatalError);
    }
...
void Foam::fvMeshTopoChangers::multiFieldRefiner2D::selectRefineCandidates
(
    PackedBoolList& candidateCells,
    const scalar lowerRefineLevel,
    const scalar upperRefineLevel,
...
Foam::scalar Foam::fvMeshTopoChangers::multiFieldRefiner2D::selectRefineCandidates
(
    PackedBoolList& candidateCells,
    const dictionary& refineDict
) const
{
    candidateCells = PackedBoolList(mesh().nCells(), 0);

    bool defaultFieldFound = false;

    // Check if refinementFields dictionary is present
    if (!refineDict.found("refinementFields"))
    {
        FatalErrorInFunction
            << "Missing refinementFields dictionary." << nl
            << exit(FatalError);
    }

    const dictionary& fieldsDicts = refineDict.subDict("refinementFields");

    // Check if any fields are specified
    if (fieldsDicts.toc().empty())
    {
        FatalErrorInFunction
            << "No fields specified for refinement." << nl
            << "Please specify fields in the dynamicMeshDict." << nl
            << exit(FatalError);
    }

    // First pass to find default field
    forAll(fieldsDicts.toc(), i)
    {
        const word fieldName(fieldsDicts.toc()[i]);
        const dictionary& fieldDict = fieldsDicts.subDict(fieldName);
        const word operation = fieldDict.lookupOrDefault<word>("operation", "default");

        if (operation == "default")
        {
            if (defaultFieldFound)
            {
                FatalErrorInFunction
                    << "Multiple default fields found. Only one allowed." << nl
                    << exit(FatalError);
            }

            // check if geometry
            const word fieldType = fieldDict.lookupOrDefault<word>("type", "scalar");

            if (fieldType == "geometry")
            {
                geometryRefineCandidates
                (
                    candidateCells,
                    fieldDict
                );
    ...
    {
        FatalErrorInFunction
            << "Default field for refinement not found." << nl
            << "Please specify one field with operation \"default\"." << nl
            << exit(FatalError);
    }

    // Second pass to combine fields
    forAll(fieldsDicts.toc(), i)
    {
        const word fieldName(fieldsDicts.toc()[i]);
        const dictionary& fieldDict = fieldsDicts.subDict(fieldName);
        ...
        if (operation != "default")
        {
            PackedBoolList tempCells(mesh().nCells(), 0);
            ...
            if (operation == "AND")
            {
                ...
                {
                    if (!tempCells.get(celli))
                    {
                        candidateCells.unset(celli);
                    }
                }
            }
    ...
    if (maxRefinement <= 0)
    {
        FatalErrorInFunction
            ...
            << "The maxCells setting in the dynamicMeshDict should be > 0." << nl
            << exit(FatalError);
    }
...
Foam::labelList Foam::fvMeshTopoChangers::multiFieldRefiner2D::selectRefineCells
(
...
) const
{
    ...
    const label nTotToRefine = (maxCells - mesh().globalData().nTotalCells())/3;
...
Foam::labelList Foam::fvMeshTopoChangers::multiFieldRefiner2D::selectUnrefinePoints
(
    const PackedBoolList& markedCell
) const
{
    // All points that can be unrefined
    const labelList splitPoints(meshCutter_.getSplitEdges());
...
void Foam::fvMeshTopoChangers::multiFieldRefiner2D::extendMarkedCells
(
...
void Foam::fvMeshTopoChangers::multiFieldRefiner2D::checkEightAnchorPoints
(
...
Foam::fvMeshTopoChangers::multiFieldRefiner2D::multiFieldRefiner2D(fvMesh& mesh, const dictionary& dict)
:
...
Foam::fvMeshTopoChangers::multiFieldRefiner2D::~multiFieldRefiner2D()
{}


// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

bool Foam::fvMeshTopoChangers::multiFieldRefiner2D::update()
{
    ...
    )
    {
...

        {
            ...
            const labelList edgesToUnrefine(meshCutter_.selectUnrefineEdges(refineCells));

            const label nSplitEdges = returnReduce
            (
        ...
        if ((nRefinementIterations_ % 10) == 0)
        {
            ...
            // Unrefinement causes holes in the refinementHistory4.
            const_cast<refinementHistory4&>(meshCutter().history()).compact();
        }
...
void Foam::fvMeshTopoChangers::multiFieldRefiner2D::topoChange(const polyTopoChangeMap& map)
{
...
void Foam::fvMeshTopoChangers::multiFieldRefiner2D::mapMesh(const polyMeshMap& map)
{
...
void Foam::fvMeshTopoChangers::multiFieldRefiner2D::distribute
(
...
bool Foam::fvMeshTopoChangers::multiFieldRefiner2D::write(const bool write) const
{
    if (changedSinceWrite_)
    {
        // Force refinement data to go to the current time directory.
        const_cast<hexRef4&>(meshCutter_).setInstance(mesh().time().timeName());
