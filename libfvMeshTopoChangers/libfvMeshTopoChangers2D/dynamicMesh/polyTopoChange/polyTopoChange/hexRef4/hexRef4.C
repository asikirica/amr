...
) const
{
    ...
    if (changed && haveTwoAnchors)
    {
        ...
        forAll(cFaces, i)
        {
            if (faceMidPoint[cFaces[i]] != faceMidPointi
                && faceMidPoint[cFaces[i]] >= 0
                && faceMidPoint[cFaces[i]] != 1234567890)
            {
                validFace = cFaces[i];
            }
        }
        ...
        label validPoint = cellAnchorPoints[celli][8 - index];
        label otherFaceMidPointi = -1;

        const face& f = mesh_.faces()[validFace];
        const labelList& fEdges = mesh_.faceEdges(validFace);
        ...
        {
            label anchorIndex = findIndex(f, validPoint);

            if (pointLevel_[f[f.rcIndex(anchorIndex)]] <= cellLevel_[celli])
            {
                otherFaceMidPointi = edgeMidPoint[fEdges[f.rcIndex(anchorIndex)]];
            }
            else
            {
                otherFaceMidPointi = f[findLevel(validFace, f, f.rcIndex(anchorIndex), false, cellLevel_[celli]+1)];
            }
            ...
            newFaceVerts.append(faceMidPoint[validFace]);
        }
        else
        {
            label anchorIndex = findIndex(f, validPoint);

            if (pointLevel_[f[f.fcIndex(anchorIndex)]] <= cellLevel_[celli])
            {
                otherFaceMidPointi = edgeMidPoint[fEdges[anchorIndex]];
            }
            else
            {
                otherFaceMidPointi = f[findLevel(validFace, f, f.fcIndex(anchorIndex), false, cellLevel_[celli]+1)];
            }
...
:
    ...
    (
        ...
        List<refinementHistory4::splitCell4>(0),
        labelList(0),
        false
    ),
    faceRemover_(mesh_, great),     // merge boundary faces wherever possible
        ...
        if (edgeMidPoint[edgeI] >= 0)
        {
            ...
            forAll(eFaces, i)
            {
                ...
                if (faceMidPoint[facei] < 0 && affectedFace.get(facei))
                {
                    ...
                    const face& f = mesh_.faces()[facei];
                    const labelList& fEdges = mesh_.faceEdges(facei, fEdgesStorage);
...
Foam::labelList Foam::hexRef4::selectUnrefineEdges
(
    const PackedBoolList& markedCell
) const
{
    // All points that can be unrefined
    const labelList splitEdges(getSplitEdges());
...
void Foam::hexRef4::topoChange(const polyTopoChangeMap& map)
{
...
void Foam::hexRef4::distribute(const polyDistributionMap& map)
{
    if (debug)
    {
...
Foam::labelList Foam::hexRef4::getSplitEdges() const
{
    ...
    if (debug)
    {
        Pout<< "hexRef4::getSplitEdges :"
            << " Calculating unrefineable edges" << endl;
    }
...
) const
{
    ...
    // conflicts.
    // maxSet = false : deselect edges to refine
    ...
    forAll(splitEdgeLabels, i)
    {
        ...
        // if (debug)
        {
            ...
            forAll(eCells, j)
            {
                ...
                {
                    FatalErrorInFunction
                        << "Initial set of split edges to unrefine does not"
...
bool Foam::hexRef4::write(const bool write) const
{
    if (cellLevel_.size() != mesh_.nCells())
    {
        FatalErrorInFunction
