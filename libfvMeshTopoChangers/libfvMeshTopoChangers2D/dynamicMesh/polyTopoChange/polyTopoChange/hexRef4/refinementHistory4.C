...
namespace Foam
{
    defineTypeNameAndDebug(refinementHistory4, 0);
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::refinementHistory4::writeEntry
(
...
void Foam::refinementHistory4::writeDebug
(
...
Foam::refinementHistory4::splitCell4::splitCell4()
:
    parent_(-1),
...
Foam::refinementHistory4::splitCell4::splitCell4(const label parent)
:
    parent_(parent),
    addedCellsPtr_(nullptr)
{}
...
Foam::refinementHistory4::splitCell4::splitCell4(Istream& is)
{
    is >> *this;
}


Foam::refinementHistory4::splitCell4::splitCell4(const splitCell4& sc)
:
    parent_(sc.parent_),
    addedCellsPtr_
    (
...
void Foam::refinementHistory4::splitCell4::operator=(const splitCell4& s)
{
...
bool Foam::refinementHistory4::splitCell4::operator==(const splitCell4& s) const
{
...
Foam::Istream& Foam::operator>>(Istream& is, refinementHistory4::splitCell4& sc)
{
...
(
    Ostream& os,
    const refinementHistory4::splitCell4& sc
)
{
...
void Foam::refinementHistory4::checkIndices() const
{
    // Check indices.
...
Foam::label Foam::refinementHistory4::allocateSplitCell
(
    const label parent,
    const label i
)
...
void Foam::refinementHistory4::freeSplitCell(const label index)
{
...
void Foam::refinementHistory4::markSplit
(
    const label index,
    labelList& oldToNew,
...
void Foam::refinementHistory4::mark
(
    const label val,
    const label index,
...
Foam::label Foam::refinementHistory4::markCommonCells
(
...
void Foam::refinementHistory4::add
(
...
void Foam::refinementHistory4::apply
(
...
Foam::refinementHistory4::refinementHistory4(const IOobject& io)
:
    regIOobject(io),
    active_(false)
{
    ...
    if (debug)
    {
        Pout<< "refinementHistory4::refinementHistory4 :"
...
Foam::refinementHistory4::refinementHistory4
(
    const IOobject& io,
    const List<splitCell4>& splitCells,
    const labelList& visibleCells,
    ...
    visibleCells_(rh.visibleCells())
{
    if (debug)
    {
        Pout<< "refinementHistory4::refinementHistory4 : constructed initial"
            << " history." << endl;
    }
}
...
(
    ...
    const UPtrList<const refinementHistory4>& refs
)
:
    regIOobject(io),
    active_(false)
{
    ...
    )
    {
        WarningIn
        (
            "refinementHistory4::refinementHistory4(const IOobject&"
            ", const labelListList&, const PtrList<refinementHistory4>&)"
    ...
    forAll(refs, refI)
    {
        const DynamicList<splitCell4>& subSplits = refs[refI].splitCells();
    ...
    forAll(refs, refI)
    {
        ...
        forAll(subSplits, i)
        {
            splitCell4& newSplit = splitCells_[offsets[refI]+i];
            ...
            {
                FixedList<label, 4>& splits = newSplit.addedCellsPtr_();
...
Foam::refinementHistory4::refinementHistory4(const IOobject& io, Istream& is)
:
    regIOobject(io),
    splitCells_(is),
    freeSplitCells_(0),
...
Foam::autoPtr<Foam::refinementHistory4> Foam::refinementHistory4::clone
(
    const IOobject& io,
...
) const
{
    ...
    return autoPtr<refinementHistory4>
    (
...
void Foam::refinementHistory4::resize(const label size)
{
    label oldSize = visibleCells_.size();

    if (debug)
    {
        Pout<< "refinementHistory4::resize from " << oldSize << " to " << size
            << " cells" << endl;
    }
...
void Foam::refinementHistory4::topoChange(const polyTopoChangeMap& map)
{
    if (active())
    {
        ...
        if (debug)
        {
            Pout<< "refinementHistory4::topoChange : from "
                << visibleCells_.size()
...
void Foam::refinementHistory4::subset
(
...
void Foam::refinementHistory4::countProc
(
    const label index,
...
void Foam::refinementHistory4::distribute(const polyDistributionMap& map)
{
    if (!active())
    {
        FatalErrorInFunction
    ...
    // If however e.g. first 1 and then the other 3 are sent across the
    ...
    // Pout<< "refinementHistory4::distribute :"
...
void Foam::refinementHistory4::compact()
{
    if (debug)
    {
        Pout<< "refinementHistory4::compact() Entering with:"
    ...
    if (debug)
    {
        Pout<< "refinementHistory4::compact : compacted splitCells from "
    ...
    if (debug)
    {
        Pout<< "refinementHistory4::compact() NOW:"
            << " freeSplitCells_:" << freeSplitCells_.size()
            << " splitCells_:" << splitCells_.size()
...
void Foam::refinementHistory4::writeDebug() const
{
    writeDebug(visibleCells_, splitCells_);
}


void Foam::refinementHistory4::storeSplit
(
    const label celli,
...
void Foam::refinementHistory4::combineCells
(
...
bool Foam::refinementHistory4::read()
{
...
bool Foam::refinementHistory4::readData(Istream& is)
{
    is >> *this;
    return !is.bad();
}
...
bool Foam::refinementHistory4::writeData(Ostream& os) const
{
    os << *this;
...
Foam::Istream& Foam::operator>>(Istream& is, refinementHistory4& rh)
{
...
Foam::Ostream& Foam::operator<<(Ostream& os, const refinementHistory4& rh)
{
    const_cast<refinementHistory4&>(rh).compact();
